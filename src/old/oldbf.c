...
//		}
			if (symbol & SYM_COUNTABLE_MASK) {
				while (p[pc] == symbol && pc < prog_len && count < MAX_COUNT) {
					count++; pc++;
				}
			}
...
PATTERN_FOUND:
		...
		for (pc = 0; pc < prog_len; pc++) {
			...
			if (p[pc] == INST_ADD) {
				x[xc - 1] += x[xc]; x[xc] = 0; continue;
			}
			if (p[pc] == INST_ADD_FWD) {
				x[xc + 1] += x[xc]; x[xc] = 0; continue;
			}
			if (p[pc] == INST_ONEBYTE && !one_byte_fields) {
				one_byte_fields = 1; fprintf(stderr, "%s: using one-byte fields\n", argv[0]); continue;
			}
			if (p[pc] == INST_QUITONEOF) {
				auto_quit_on_eof = 1; continue;
			}
			...
			// '<'
			else if (symbol == SYM_PD) {
				xc -= count;
			}
			// '+'
			else if (symbol == SYM_DI) {
				x[xc] += count;
			}
			// '-'
			else if (symbol == SYM_DD) {
				x[xc] -= count;
			}
			// '.'
			//else if (symbol == SYM_OUT) putchar(x[xc]);
			else if (symbol == SYM_OUT) {
				fputc(x[xc], out_stream), fflush(out_stream);
			}
			// ','
			...
			// '['
				...
				if (x[xc] == 0 || (one_byte_fields && (x[xc] & 0xFF) == 0)) {
					...
					while (l > 0 || p[pc] != SYM_BE) {
						if (get_symbol(p[pc]) == SYM_BB) {
							l++;
						} else if (get_symbol(p[pc]) == SYM_BE) {
							l--;
						}
						pc++;
					}
			...
			// ']'
				...
				while (l > 0 || p[pc] != SYM_BB) {
					if (get_symbol(p[pc]) == SYM_BE) {
						l++;
					} else if (get_symbol(p[pc]) == SYM_BB) {
						l--;
					}
					pc--;
				}
