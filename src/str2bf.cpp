...
void calculate_pool(char* pool, int &poolsize, const string& str)
{
...
#if 1
	{
		int newpos = 0;
		for (int i = 0; i < poolsize; i++) {
			if (abs(pool[i] - str[0]) < abs(pool[newpos] - str[0])) {
				newpos = i;
			}
		}
...
//	calculate_pool(pool,poolsize,str);
	...
	for (iter = str.begin(); iter != str.end(); ++iter) {
		...
		// Find closest value
		for (int i = 0; i < poolsize; i++) {
			if (abs(pool[i] - *iter) + abs(i - cur_pos) / 2 < abs(pool[newpos] - *iter)) {
				newpos = i;
			}
		}
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1) {
		...
		for (iter = code.begin(); iter != code.end(); ++iter) {
			if (cur_col > line_width) {
				cout << endl, cur_col = 0, lines++;
			}
			cur_col++;
			cout << *iter;
		}
			   ...
			   string line;
			   do { char tmp(cin.get()); if (!cin.eof()) {
						line += tmp;
					}
			   } while (!cin.eof() && line[line.size() - 1] != '\n');
			   //getline(cin,line);
			   //line+='\n';
